...
def test(test_loader, tnet):
    ...
    path = "/content/drive/MyDrive/sample_sublook/4735"
    imfilelist = [os.path.join(path, f) for f in os.listdir(path) if f.endswith(imageformat)]

    sublook_images = []
    for el in imfilelist:
        image = Image.open(el)
        sublook_images.append(image)
    ...
    sublook_dataset = []
    for sublook_img in sublook_images:
        print(sublook_img.filename)
        sublook_dataset.append(transform(sublook_img))

    sublook_loader = torch.utils.data.DataLoader(sublook_dataset)

    for batch_idx, images in enumerate(sublook_loader):
        print(images.shape)
        if args.cuda:
            images = images.cuda()
        images = Variable(images)
    ...
    outfit_vec = torch.nn.functional.pairwise_distance(embeddings[0].unsqueeze(0), embeddings[1].unsqueeze(0), 2)
    print(embeddings)
    # print(embeddings[0], embeddings[0].unsqueeze(0).unsqueeze(0))
    print("one sample outfit score", torch.norm(outfit_vec))
